...
if(!isset($_COOKIES['token'])){
    $curl = curl_init("https://karmamobileapps.herokuapp.com/generate-token");
...
<div id="contained_content" style="<?php echo $margin; ?>">
	...
	<section id="landing-page__form-header" class="section-wrapper">
        ...
        <p>Simply complete the entry form below, to receive your $300 worth of vouchers and enter the prize draw. </p>
    </section>
  ...
  <div class="remodal" data-remodal-id="modal" role="dialog" aria-labelledby="modal1Title" aria-describedby="modal1Desc" id="modal-success">
      ...
      <div>
          ...
          <p id="modal1Desc" style="color: #000000;">
			...
			Thank you for entering the London Sevens prize draw to WIN 7 nights at any Karma Resort or Retreat.<br>
			Good luck!<br><br>
			You will shortly receive email confirmation, including a link to download your exclusive $300 worth of vouchers.
          </p>
      </div>
      <br>
...
$(document).ready(function($) {
	...
	$("#form-helper").on("submit", function(e){
		...
		if(isHelperValid){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo get_karma_api_base_url(); ?>/seven-helper",
				headers: {
 ...
 $('#frm').on('submit',function(e){
	...
	checkingPhone(function(){
		if($("#agreecode").is(":checked")){
			if($("#doNotCall").is(":checked")){							
				if (isvalidate && isValidPhone) {                              					
					...
					if(userAge >= 18){						
						$.ajax({
							type: "GET",
							url: "https://karmawarehouse.izenoondemand.com/production/api/sugarapi/emailpromocodelookup-wh.php?email="+$('#email1').val()+"&pc=7-RFU",
							...
							if (data == '0'){
								var dataPush = { 
									...
									'collectionEntryPoint': "london sevens",
								...
								$.ajax({
									type: "POST",
									url: "<?php echo get_karma_api_base_url(); ?>/seven-post",
									data: dataPush,
									dataType: "json",
								...
								$('#alert-desc').html("<p style=\"margin-top: 15px;\">You have already entered the promotion, 1 entry per person only. See Terms and Conditions <a style=\"color: #8d7249; text-decoration: none;\" target=\"_blank\" href=\"<?php echo get_site_url().'/sevens-terms-and-conditions'?>\">here</a>.</p>");
								$('#modal-alert').remodal().open();
								$('#loading').css('display','none');
								$("#submitbookingbutton").prop("disabled", false);
							}
						...
						$('#alert-desc').html("<p style=\"margin-top: 15px;\">Please note, this prize draw is open to people aged 18 and over only. See Terms and Conditions <a style=\"color: #8d7249; text-decoration: none;\" href=\"<?php echo get_site_url().'/sevens-terms-and-conditions' ?>\" target=\"_blank\">here</a>.</p>");
						$('#modal-alert').remodal().open();
						$('#loading').css('display','none');
						$("#submitbookingbutton").prop("disabled", false);
					}	   	
			...
			$('#alert-desc').html("<p style=\"margin-top: 15px;\">Please confirm you have read and agree to the <a style=\"color: #8d7249; text-decoration: none;\" href=\"<?php echo get_site_url().'/sevens-terms-and-conditions' ?>\" target=\"_blank\">Terms and Conditions</a>.</p>");
			$('#modal-alert').remodal().open();
			$('#loading').css('display','none');
			$("#submitbookingbutton").prop("disabled", false);
		}
