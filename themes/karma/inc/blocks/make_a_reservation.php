...
$booking_link_action_mobile = get_post_meta(get_the_ID(), 'booking_url', 1);
if ($booking_link_action_mobile == false) {
	$booking_link_action_m = 'onclick="showEnquiryForm();"';
} else {
	if(get_the_ID() == 5825){
   		$booking_link_action_m = 'href="https://uk2.roomlynx.net/rezrooms2/loadOBMApplication.action?siteId=SANCSOHO&chainAction=newAvailabilitySearch&request_locale=en"';
	} else {
		$booking_link_action_m = 'href="'.$booking_link_action_mobile.'"';
	}	
}
...
<div id="make_reservation">
	<div class="cta_banner" style="background: url('<?php echo $background_image_url; ?>') no-repeat; background-position: center; background-size: cover;">
		...
		<div class="content">
			...
			<?php if( $timeshare_resort ): ?>
        <h4 style="color:<?php echo $text_color; ?>;"><p><?php  echo $fields['description']; ?></p></h4>
      <?php else: ?>
			<h4 style="color:<?php echo $text_color; ?>;"><p><a href="<?php echo get_option('home'); ?>/contact-us/?pid=<?php echo get_the_ID(); ?>" style="color: #fff; border-bottom: 1px solid #fff;" title="Contact Us">Contact us</a> now and we’ll tailor your perfect getaway.</p><p>&nbsp;</p><!-- <?php  echo nl2br($fields['description']); ?> --></h4>
		<?php endif; ?>	
<a style="cursor:pointer;" <?php echo $booking_link_action;?> 
...
<a style="cursor:pointer;" <?php echo $booking_link_action_m;?> 
