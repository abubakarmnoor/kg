...
<div id="contained_content" style="<?php echo $margin; ?>">
	...
	<section id="landing-page__form-header" class="section-wrapper">
        <p id="landing-page__form-header__title">How to Enter</p>
        <p style="max-width: 1200px; margin: auto;">
		SIMPLY SHARE AN IMAGE OF A <span style="color: #9c7949;">FEEL GOOD MOMENT</span> USING THE HASHTAG <span style="color: #9c7949;">#FeelGoodKarma</span> ON INSTAGRAM, FOLLOW <span style="color: #9c7949;"><a href="https://www.instagram.com/feelgooddrinks/" target="_blank">@FeelGoodDrinks</a> <a href="https://www.instagram.com/karma.group/" target="_blank">@Karma.Group</a></span> and <a href="https://www.instagram.com/leannelimwalker/?hl=en" target="_blank"><span style="color: #9c7949;">@LeanneLimWalker</span></a>, AND COMPLETE THE ENTRY FORM BELOW, TO RECEIVE YOUR VOUCHER AND ENTER THE PRIZE DRAW. 		
    </section>
		...
		<div class="container container-inner">
			...
			<div class="row">
					...
					<div class="col-xs-12 col-sm-6">
						...
						<span style="font-size: 18px;position: absolute;top: 30px;">@</span>
							<input type="text" class="form-control" id="instagramAcc" name="instagram" placeholder="Instagram**" required style="padding-left: 20px;">
						</div>
					</div>
			</div>
						...
						<div class="row">							
							<div class="form-group button-wrap" style="padding: 0;">
  ...
  <div class="remodal" data-remodal-id="modal" role="dialog" aria-labelledby="modal1Title" aria-describedby="modal1Desc" id="modal-success">
      ...
      <div>
          <p id="modal1Desc" style="color: #000000;">
			...
			Thank you for entering the Feel Good with Karma prize draw to WIN a holiday to Karma Kandara, Bali. <br>
			Good luck!<br><br>
			You will shortly receive email confirmation, including a link to download your £250 voucher to use against booking accommodation at any Karma Resort or Retreat.
          </p>
      </div>
      <br>
...
$(document).ready(function($) {		
	...
	$("#form-helper").on("submit", function(e){
		...
		if(isHelperValid){
			...
			$.ajax({
				type: "POST",
				url: "<?php echo get_karma_api_base_url(); ?>/feel-good-helper",
				data: {
...
function checkingPhone(callback, flag){
 ...
 $('#frm').on('submit',function(e){
	 ...
	 var instagramAcc = $("#instagramAcc").val();
			...
			if($("#agreecode").is(":checked")){							
				if ($("#doNotCall").is(":checked")) {                              					
					...
					if(userAge >= 18){						
						$.ajax({
							type: "GET",
							url: "https://karmawarehouse.izenoondemand.com/production/api/sugarapi/emailpromocodelookup-wh.php?email="+$('#email1').val()+"&pc=FEEL-GOOD",
							...
							if (data == '0'){
								var dataPush = { 
									...
									"website": "https://www.instagram.com/" + instagramAcc,
									'collectionEntryPoint': "Feel Good Promotion Page",
								...
								$.ajax({
									type: "POST",
									url: "<?php echo get_karma_api_base_url(); ?>/feel-good-post",
									data: dataPush,
									dataType: "json",

								}).done(function(data) {
										var urlWarehouse = 'https://karmawarehouse.izenoondemand.com/production/api/karmaclub/group/karmacluboffer.php?title='+title+'&first_name=' + firstname + '&last_name=' + lastname + '&email=' + email + '&phone=' + phone + '&dob=' + dob2 + '&country=' + country + '&postalcode=' + postcode + '&maritalStatus=' + marital + '&collection_entry_point=Feel Good Promotion Page&website=https://www.instagram.com/' + instagramAcc + '&promocode=FEEL-GOOD&card_downloaded_c=false';
								...
								$('#alert-desc').html("<p style=\"margin-top: 15px;\">You have already entered the promotion, 1 entry per person only. See Terms and Conditions <a style=\"color: #8d7249; text-decoration: none;\" target=\"_blank\" href=\"<?php echo get_site_url().'/feelgood-terms-and-conditions'?>\">here</a>.</p>");
								$('#modal-alert').remodal().open();
								$('#loading').css('display','none');
								$("#submitbookingbutton").prop("disabled", false);
							}
						...
						$('#alert-desc').html("<p style=\"margin-top: 15px;\">Please note, this prize draw is open to people aged 18 and over only. See Terms and Conditions <a style=\"color: #8d7249; text-decoration: none;\" href=\"<?php echo get_site_url().'/feelgood-terms-and-conditions' ?>\" target=\"_blank\">here</a>.</p>");
						$('#modal-alert').remodal().open();
						$('#loading').css('display','none');
						$("#submitbookingbutton").prop("disabled", false);
					}	   	
				...
				$('#alert-desc').html("<p style=\"margin-top: 15px;\">Please confirm you have read and agree to the <a style=\"color: #8d7249; text-decoration: none;\" href=\"<?php echo get_site_url().'/feelgood-terms-and-conditions' ?>\" target=\"_blank\">Terms and Conditions</a>.</p>");
				$('#modal-alert').remodal().open();
				$('#loading').css('display','none');
				$("#submitbookingbutton").prop("disabled", false);
			}			
