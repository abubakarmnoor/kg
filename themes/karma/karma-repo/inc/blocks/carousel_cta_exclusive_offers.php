...
foreach ($fields['slides'] as $slide) {
	...
	if (!empty($slide['title'])) {
		$html .= '<h2 class="georgia_font italic">' .$slide['title']. '</h2>';
	}
	if (!empty($slide['description'])) {
		$html .= $slide['description'];
	}
	if (!empty($link)) {
		$html .= '<a class="button_outline" href="' .$link. '" target="_blank" title="' .$slide['link_text']. '" title="' .$slide['link_text']. '" alt="' .$slide['link_text']. '" style="margin-bottom: 8px;">' .$slide['link_text']. '</a>';
	}
